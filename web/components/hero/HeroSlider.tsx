    ...
    <section className="container">
      <div className="relative overflow-hidden rounded-[32px] border border-[var(--border)] bg-white shadow-card" dir="rtl">
        <div className="relative h-[420px] w-full">
          <Image src={slide.image} alt={slide.title} fill className="object-cover" priority={active === 0} sizes="100vw" />
          ...
          <div className="relative z-10 flex h-full flex-col justify-center gap-4 px-6 py-10 md:px-14">
            ...
            <h1 className={`text-3xl font-extrabold leading-relaxed md:text-4xl ${lightText ? "text-slate-900" : "text-white"}`}>
              {slide.title}
            </h1>
            <p className={`max-w-2xl text-base ${lightText ? "text-slate-600" : "text-slate-100"}`}>{slide.summary}</p>
            <button className="w-fit rounded-full bg-brand px-6 py-3 text-sm font-bold text-white shadow-md transition hover:shadow-lg">
              مشاهده گزارش کامل
            </button>
          </div>
        </div>
        <div className="flex flex-wrap items-center justify-between gap-4 border-t border-[var(--border)] bg-white px-6 py-4" dir="rtl">

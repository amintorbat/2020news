...
function FormIndicator({ form }: { form?: ("W" | "D" | "L")[] }) {
  ...
  return (
    <div className="flex gap-1.5 items-center justify-center h-full" title="۵ بازی اخیر">
      {form.slice(0, 5).map((result, idx) => {
        ...
        return (
          <span
            key={idx}
            className={`w-7 h-7 rounded-full ${color} flex-shrink-0 flex items-center justify-center font-bold text-xs ${textColor}`}
          >
            {result}
          </span>
        );
        ...
        <table className="w-full text-sm">
          <thead className="bg-[#f7f8fa]">
            <tr>
              ...
              <th className="px-4 py-3 text-center font-semibold text-slate-900 whitespace-nowrap"></th>

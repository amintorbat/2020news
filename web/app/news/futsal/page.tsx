...
export async function generateMetadata(): Promise<Metadata> {
  return {
    title: "اخبار فوتسال | 2020news",
    description: "آخرین اخبار، گزارش‌ها و تحلیل‌های فوتسال ایران و جهان",
  };
}

type NewsFutsalPageProps = {
  searchParams?: {
    q?: string;
...
export default async function NewsFutsalPage({ searchParams }: NewsFutsalPageProps) {
  ...
  // Filter by futsal
  let filtered = latestNews.filter((article) => article.sport === "futsal");
      ...
      <div className="space-y-8">
        ...
        <section className="container pt-8" dir="rtl">
          <div className="space-y-4">
            ...
            <div>
              <h1 className="text-3xl font-extrabold text-slate-900 sm:text-4xl">اخبار فوتسال</h1>
              <p className="mt-2 text-sm text-slate-600 sm:text-base">
                آخرین اخبار، گزارش‌ها و تحلیل‌های فوتسال ایران و جهان
              </p>
            </div>
                ...
                <>
                  <div className="flex items-center justify-between">
                    <h2 className="text-xl font-bold text-slate-900">
                      {query ? `نتایج جستجو برای "${query}"` : "همه اخبار فوتسال"}
                    </h2>
...
function buildPaginationUrl(
  ...
  return `/news/futsal${queryString ? `?${queryString}` : ""}`;
}
